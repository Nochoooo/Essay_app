...
for msg in consumer:
    try:
        ...
        if content:
            score = model.predict_score(content)
            response = {
                'score': score,
                'file_path': file_path
            }
            producer.send('predict_responses', response)
            logger.info(f"Отправлен результат: {response}")
        else:
            logger.error("Получено сообщение без содержания.")
