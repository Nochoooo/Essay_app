...
class Database:
    ...
    def insert_data_into_essays(self, author, file_path, score):
        try:
            self.connect()
            self.cursor.execute("INSERT INTO essays (author, file_path, score) VALUES (%s, %s, %s)", (author, file_path, score))
            self.conn.commit()
    ...
    def get_all_essays(self):
        try:
            ...
            return [{'id': essay[0], 'author': essay[1], 'file_path': essay[2], 'score': essay[3]} for essay in essays]
